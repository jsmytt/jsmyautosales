...
def car_detail(request,car_slug):
    ...
    query2 = "select listing_carimg.limage,listing_car.slug from listing_car inner join listing_carimg on listing_car.id = listing_carimg.car_id where listing_car.slug = \'" + str(car_name) + "\' ORDER BY listing_car.id desc, listing_carimg.id"
...
def faq_detail(request,car_slug):
    ...
    car_name = re.sub(".*[s]+[l]+[u]+[g]+[\%]+[3]+[D]", "", str(request.build_absolute_uri()))
    query1 = "select listing_car.created,listing_car.body ,listing_car.sold, listing_car.price,listing_car.slug, listing_car.title from listing_car where listing_car.slug = \'" + str(car_name) + "\' ORDER BY listing_car.id"
    cursor.execute(query1)
    df = dictfetchall(cursor)
    ...
    query3 = "select listing_faqath.attachment, listing_faqath.id ,listing_car.slug from listing_car inner join listing_faqath on listing_car.id = listing_faqath.car_id where listing_car.slug = \'" + str(car_name) + "\' ORDER BY listing_car.id desc"
